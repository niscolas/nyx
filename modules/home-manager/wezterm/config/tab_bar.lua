...
M.setup = function(color_scheme)
    ...
    function tab_title(tab_info)
        ...
        local next_slash_idx = string.find(title, "/")
        if next_slash_idx ~= nil then
            title = string.sub(title, next_slash_idx, string.len(title))
        end
    ...
    wezterm.on(
        ...
        function(tab, tabs, panes, config, hover, max_width)
            ...
            local index_bg = tab.is_active and color_scheme.brights[4]
                or color_scheme.brights[5]
